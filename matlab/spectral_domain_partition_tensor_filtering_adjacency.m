...
function [ P ] = getadjacency_tensor( tensor, weights, fun, sigma )
%GETADJACENCY_TENSOR Returns the weighted adjacency matrix sum
%   Detailed explanation goes here
    s = size(tensor);
    P = zeros(s(1), s(2));
    ...
    % Make a weighted sum
    for i=1:s(3)
        P = P + (weights(i) * fun(tensor(:,:,i), sigma));
    end;
end
...
%   Detailed explanation goes here
    ...
    % Get the average adjacency matrix
    P = getadjacency_tensor(tensor, weights, fun, sigma);
    
    % Calculate the normalized random walk laplacian as usual
    T = diag(sum(P, 2));
    L = eye(size(P)) - T\P;
end
