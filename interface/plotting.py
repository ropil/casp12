...
def correlate_methods_global(methods_ids, methods_id2name, database):
    ...
    methods = sorted(list(methods_ids))
    names = [methods_id2name[method] for method in methods]
    query = query_global_correlates(methods)
    correlates = remove_model_column(database.execute(query).fetchall())b
    return get_dataframe(correlates, names)