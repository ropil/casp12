...
def pcons_domain_specifications(casp, target, database):
    query = 'SELECT len FROM target WHERE casp="{}" AND id="{}";'.format(casp, target)
    target_length = database.execute(query).fetchone()[0]
    ignore_residues = {}

    # Sum domain lengths if target length not specified
    if target_length is None:
        query = "SELECT SUM(dlen) FROM domain_size WHERE casp={} AND target='{}' GROUP BY casp, target;".format(casp, target)
        target_length = database.execute(query).fetchone()[0]
    ...
    for (domain,) in database.execute(query):
        ...
        domain_residues = set()
        query = "SELECT start, stop FROM segment WHERE casp={} AND target='{}' AND domain={};".format(casp, target, domain)
